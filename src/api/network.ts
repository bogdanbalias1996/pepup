import { ApiMethod, CitiznApi, ICitiznApi } from './api'
...
const getObservable = (config: CitiznApi): Promise<any> => {
  ...
  switch (method) {
    ...
    case ApiMethod.PUT:
      return addHeaders(agent.put(url).query(queryParams).send(data)).then(getResponse)
...
export const request = (options: ICitiznApi) => {
  const ApiConfig = new CitiznApi(options)
